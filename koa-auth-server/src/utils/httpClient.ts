...
export class HttpClient {
    ...
    constructor(baseUrl: string, serviceName: string, options: Partial<OptionsOfJSONResponseBody> = {}) {
     ...
     * GET 요청 수행
     */
    public async get<T = any>(endpoint: string, options: Partial<OptionsOfJSONResponseBody> = {}): Promise<T> {
        try {
     ...
     * POST 요청 수행
     */
    public async post<T = any>(endpoint: string, data?: unknown, options: Partial<OptionsOfJSONResponseBody> = {}): Promise<T> {
        try {
     ...
     * PUT 요청 수행
     */
    public async put<T = any>(endpoint: string, data?: unknown, options: Partial<OptionsOfJSONResponseBody> = {}): Promise<T> {
        try {
            const { json: _, ...restOptions } = options as any;
     ...
     * DELETE 요청 수행
     */
    public async delete<T = any>(endpoint: string, options: Partial<OptionsOfJSONResponseBody> = {}): Promise<T> {
        try {
    ...
    private getErrorMessage(error: HTTPError): string {
        ...
        switch (error.response.statusCode) {
            ...
            case 429: return '너무 많은 요청을 보냈습니다';
            case 500: return '서버 내부 오류가 발생했습니다';
            default: return `${error.response.statusCode} 오류가 발생했습니다`;
        }
    }
}