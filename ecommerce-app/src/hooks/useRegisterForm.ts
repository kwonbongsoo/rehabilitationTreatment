...
class RegisterDataTransformer {
    ...
    static addIdempotencyKey(data: RegisterRequest, idempotencyKey: string): RegisterRequest & { _idempotencyKey: string } {
        return {
            ...data,
...
export function useRegisterForm(): UseRegisterFormReturn {
    ...
    const executeRegistration = useCallback(async (registerRequest: RegisterRequest): Promise<any> => {
        return await idempotentMutation.executeMutation(
            async (data: RegisterRequest, idempotencyKey: string) => {
                const requestWithIdempotency = RegisterDataTransformer.addIdempotencyKey(data, idempotencyKey);
    ...
    // 개발 환경에서만 디버깅 정보 제공
    const developmentProps = process.env.NODE_ENV === 'development' ? {
        requestStatus,
