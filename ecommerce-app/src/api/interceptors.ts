...
class ApiLogger implements ILogger {
  ...
  logError(request: any, error: unknown): void {
    ...
    const errorData = error instanceof AxiosError ? error.response?.data || error.message : error;

    console.error(`❌ API Error: ${request?.method?.toUpperCase()} ${request?.url}`, errorData);
  }
...
class InterceptorManager {
  ...
  constructor(logger: ILogger, authService: IAuthService, idempotencyService: IIdempotencyService) {
  ...
  private setupRequestInterceptor(client: AxiosInstance): void {
    client.interceptors.request.use(this.requestInterceptor.handle, (error: AxiosError) =>
